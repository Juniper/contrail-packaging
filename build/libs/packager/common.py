...
class BasePackager(Utils):
    ...
    def retrieve_pkgtypes_recursively(self, pkgtypes):
        ...
        for pkgtype in pkgtypes:
            if (pkgtype in self.contrail_pkgs_dict.keys() and
                ...
                sub_pkgs = self.contrail_pkgs_dict[pkgtype][pkgtype]['sub_package_type']
    ...
    def rearrange_types(self, pkgtypes):
        ...
        for pkgtype in pkgtypes:
            ...
            if pkgtype not in pkgs_dict:
                raise RuntimeError('No packages are tagged with Package type (%s)' % pkgtype)
            ...
            if 'sub_package_type' in pkgs_dict[pkgtype][pkgtype]:
                ...
                for sub_type in filter(None, sub_package_types):
                    ...
                    if sub_type not in new_pkgtypes:
    ...
    def ks_build(self):
        ...
        if self.package_types:
            pkgtypes = self.retrieve_pkgtypes_recursively(self.package_types)
        else:
