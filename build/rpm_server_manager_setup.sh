...
if [ "$WEBUI" != "" ]; then
  ...
  if [ $? == 0 ]; then
    sed -i "s/config.featurePkg.serverManager.enable = .*/config.featurePkg.serverManager.enable = true;" >> $WEBUI_CONF_FILE
  else
  ...
  if [ $? == 0 ]; then
    sed -i "s/config.orchestration = .*/config.orchestration = {};" >> $WEBUI_CONF_FILE
  else
  ...
  if [ $? == 0 ]; then
    sed -i "s/config.multi_tenancy = .*/config.multi_tenancy = {};" $WEBUI_CONF_FILE
  else
    echo "config.multi_tenancy = {};" >> $WEBUI_CONF_FILE
  fi
  grep "config.multi_tenancy.enabled" $WEBUI_CONF_FILE
  if [ $? == 0 ]; then
    sed -i "s/config.multi_tenancy.enabled = .*/config.multi_tenancy.enabled = false;" $WEBUI_CONF_FILE
  else
