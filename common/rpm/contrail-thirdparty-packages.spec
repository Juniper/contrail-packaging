...
%endif
%{echo: "Building contrail-thirdparty-packages for release %{_relstr}\n"}
%if 0%{?_fileList:1}
...
# install files
install -p -m 755 %{_builddir}/../tools/packaging/build/contrail-thirdparty.repo %{buildroot}/etc/yum.repos.d/contrail-thirdparty.repo
if [ -f %{_flist} ]; then echo "Using TGZ FILE = %{_flist}"; tar xzf %{_flist} -C %{buildroot}%{_contrailopt}/contrail_thirdparty_repo/ 2>/dev/null; else echo "ERROR: TGZ file containing all rpms is not supplied or not present"; echo "Supply Argument: FILE_LIST=<TGZ FILE>"; exit 1; fi

%files
%defattr(-, root, root)
%{_contrailopt}/contrail_thirdparty_repo/*
/etc/yum.repos.d/contrail-thirdparty.repo
