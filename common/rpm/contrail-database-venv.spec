...
%endif
%{echo:     "Building venv contrail database release %{_relstr}\n"}
%if 0%{?_srcVer:1}
...
Summary:    Contrail database virtual env %{?_gitVer}
Name:       contrail-database-venv
Version:	    %{_verstr}
License:    Commercial
...
%description
Creates virtual env for contrail database role
...
%define _target %{buildroot}/opt/contrail/database-venv
...
%files
%defattr(-,root,root,-)
/opt/contrail/database-venv

%post
pushd /opt/contrail/database-venv/
rm -rf lib64
ln -sf lib lib64
cd bin
