...
%endif
%{echo:     "Building venv contrail vrouter release %{_relstr}\n"}
%if 0%{?_srcVer:1}
...
Summary:    Contrail Vrouter virtual env %{?_gitVer}
Name:       contrail-vrouter-venv
Version:	    %{_verstr}
License:    Commercial
...
%description
Creates virtual env for contrail vrouter role
...
%define _target %{buildroot}/opt/contrail/vrouter-venv

# start venv
pushd %{_builddir}/virtualenv-1.9.1
python virtualenv.py %{?_vpy} --extra-search-dir=$(pwd)/reqs --extra-search-dir=$(pwd)/virtualenv_support --never-download --system-site-packages %{_target}
popd
...
%files
%defattr(-,root,root,-)
/opt/contrail/vrouter-venv

%post
pushd /opt/contrail/vrouter-venv/
rm -rf lib64
ln -sf lib lib64
cd bin
