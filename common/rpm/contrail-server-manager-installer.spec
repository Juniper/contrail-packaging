...
if [ -f %{_flist} ]; then echo "Using TGZ FILE = %{_flist}"; cp %{_flist} %{buildroot}%{_contrailopt}/contrail_server_manager/packages/; else echo "ERROR: TGZ file containing all rpms is not supplied or not present"; echo "Supply Argument: FILE_LIST=<TGZ FILE>"; exit 1; fi

%preun
rm -rf /opt/contrail/contrail_server_manager/packages/*
...
%files
%defattr(-, root, root)
%{_contrailopt}/contrail_server_manager/README
%{_contrailopt}/contrail_server_manager/setup.sh
%{_contrailopt}/contrail_server_manager/packages/*
