...
class EventManager:
	...
	if (self.node_type == 'contrail-database'):
        ...
        if (self.node_type == 'contrail-database') and (os.path.exists("/var/lib/cassandra/data/ContrailAnalytics")):
            (os_is, x, y) = platform.dist()
            if (os_is == 'Ubuntu'):
            ...
            if (disk_space_used is not None) and (disk_space_available is not None) and (analytics_db_size is not None):
